...
export default function FindOutMoreEveryday({ onClose }) {
    const { current, updateWTEFavourites } = useSaveSlots();
    const favorites = new Set(current?.wteFavourites?.[2] || []);
    const [selectedCategory, setSelectedCategory] = useState('All');

    const partnerBrands = [
        { id: '1', name: 'Woolworths Supermarkets', category: 'Supermarket', desc: 'Shop your everyday groceries and earn Rewards points.' },
        { id: '2', name: 'BIG W', category: 'Retail', desc: 'Earn points on homewares, clothing, toys and everyday essentials.' },
        { id: '3', name: 'BWS', category: 'Liquor', desc: 'Stock up on beer, wine, and spirits while boosting your points balance.' },
        { id: '4', name: 'Everyday Market', category: 'Retail', desc: 'Shop an extended range of products and brands online at Woolworths.' },
    ];

    const displayedPartners = selectedCategory === 'All'
        ? partnerBrands
        : partnerBrands.filter(p => p.category === selectedCategory);

    const toggleFav = (itemId) => {
        updateWTEFavourites(2, String(itemId));
    };
        ...
        <div className="w-full h-full flex flex-col items-stretch overflow-hidden bg-white text-left">
            ...
            <div className="flex-grow p-6 overflow-y-auto">
                ...
                <div className="flex items-center justify-between mb-4">
                    <h4 className="text-[16px] font-bold text-[#323232]">Participating Brands</h4>
